...
void DenseLossLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  const int dense_dims = this->layer_param_.dense_loss_param().dense_dims();
  const float dense_weight = this->layer_param_.dense_loss_param().dense_weight();
  
  const int axis = bottom[0]->CanonicalAxisIndex(
      this->layer_param_.dense_loss_param().axis());
  // Dimensions starting from "axis" are "flattened" into a single
  // length K_ vector. For example, if bottom[0]'s shape is (N, C, H, W),
  // and axis == 1, N inner products with dimension CHW are performed.
  K_ = bottom[0]->count(axis);

  CHECK_EQ(bottom[1]->channels(),dense_dims);
  CHECK_EQ(K_,dense_dims);
...
template <typename Dtype>
void DenseLossLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void DenseLossLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
  ...
  {
    // D(i,:) = X(i,:) - Label(i,:)
    caffe_sub(K_, bottom_data+i*K_, label+i*K_, distance_data+i*K_);
  }
  Dtype dot = caffe_cpu_dot(M_ * K_, distance_.cpu_data(), distance_.cpu_data());
  Dtype loss = dot / M_ / Dtype(2);
  top[0]->mutable_cpu_data()[0] = loss/128;
}
